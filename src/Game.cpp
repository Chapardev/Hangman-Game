...
{
    m_texts["attempts"] = gf::Text { "Attempts: " + std::to_string(m_attempts), m_font, 24 };
    m_texts.at("attempts").setPosition( { 15.f, m_texts.at("attempts").getCharacterSize() + 10.f } );
    m_texts.at("attempts").setColor(gf::Color::White);
    ...
    m_texts["underscores"] = gf::Text { m_texts.at("word") };
    m_texts.at("underscores").setLetterSpacing(m_texts.at("word").getLetterSpacing());
    m_texts.at("underscores").setPosition( { m_texts.at("word").getPosition().x + 3.f, m_texts.at("word").getPosition().y } );
    m_texts.at("underscores").setColor(gf::Color::White);
...
void Game::checkLetter(char letter)
{
    ...
    {
        m_attempts--;
        m_texts.at("attempts").setString("Attempts: " + std::to_string(m_attempts));
